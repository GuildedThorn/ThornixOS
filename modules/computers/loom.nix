...
in
{
  flake.nixosConfigurations.loom = inputs.nixpkgs.lib.nixosSystem {
    ...
    modules = [
      ...
      "${inputs.self}/hosts/loom/hardware-configuration.nix"
      ...
      (
        { lib, ... }:
        {
          # Workflow credentials make Loom a privileged automation host.
          # Audit all service execution and keep recovery strictly key-only.
